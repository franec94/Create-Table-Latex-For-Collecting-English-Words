...
def prepare_preprocess_input_data(val) -> object:
  ...
  english_words = list(
    map(lambda xi: padd(xi, max_len), target_words)
    )
  val = list()
...
def _check_input_data_process_data(input_data, input_names) -> None:
  ...
  for index, val in index_data_int:
    assert val <= 0, f'{name[index]} is negative or zeroed'
  pass
...
def write_results(results: list, header_items: list):
  table_funcs.write_tables_2_file(results, header_items)
  pass